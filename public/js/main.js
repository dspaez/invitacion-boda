...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const asistentesGuardadosBlock = document.getElementById("asistentes-guardados-block");
    ...
    btn?.addEventListener("click", async () => {
        ...
        if (!codigo) {
            errorMsg.textContent = "⚠️ Ingresa tu código de invitación. Si no lo tienes, por favor solicítalo.";
        ...
        try {
            ...
            errorMsg.textContent = "❗ Hubo un problema al verificar tu código. Intenta de nuevo más tarde.";
            errorMsg.classList.remove("hidden");
        }
    });
    ...
    rsvpForm?.addEventListener("submit", async (e) => {
        ...
        // Validaciones
        if (!respuesta) {
            errorField.textContent = "Selecciona una opción para confirmar tu asistencia.";
        ...
        if (respuesta === "Sí asistiré") {
            if (isNaN(asistentes) || asistentes <= 0) {
                errorField.textContent = "Por favor ingresa la cantidad de asistentes (mínimo 1).";
                errorField.classList.remove("hidden");
                return;
            }
        }
        ...
        if (!codigoActual) {
            errorField.textContent = "Hubo un error interno con el código. Intenta verificar nuevamente.";
            errorField.classList.remove("hidden");
            return;
        }
...
function iniciarCuentaRegresiva() {
    const destino = new Date("2025-08-15T15:00:00-05:00").getTime();
...
document.querySelector(".btn-sugerir-cancion")?.addEventListener("click", () => {
